...
(function($)
{
	$.fn.wScratchPad = function(option, settings)
	{
		...
		return this.each(function()
		{
			...
			if(!test.getContext)
			{
				elem.html("Votre navigateur ne prend pas en charge le HTML5, veuillez utiliser un navigateur plus récent.");
				return false;	
			}
	...
	ScratchPad.prototype = 
	{
		generate: function()
		{
			...
			this.sp =
			$('<div class="scratchthis"></div>')
			.css({position: 'relative', background: this.settings.color})
			.append(
				$(this.canvas)
				.attr('width', this.settings.width + 'px')
				.attr('height', this.settings.height + 'px'),
				$('<div class="scratchcontent '+ this.settings.classscratch +'"><div class="first">'+ this.settings.firsttext +'</div><div class="middle">'+ this.settings.middletext +'</div><div class="last">Cliquer sur <span>'+ this.settings.lasttext +'</span></div></div>')
			)
